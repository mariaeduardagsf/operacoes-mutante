...
describe("Suíte de Testes Fraca para 50 Operações Aritméticas", () => {
  ...
  test("6.1 deve calcular a raiz quadrada de 0 (caso de borda)", () => {
    expect(raizQuadrada(0)).toBe(0);
  });
  test("6.2 deve lançar erro ao calcular a raiz quadrada de número negativo", () => {
  ...
  test("10.1 deve calcular o produto de um array vazio como 1", () => {
  ...
  test("33.1 deve retornar false para n <= 1 (não primo)", () => {
    expect(isPrimo(1)).toBe(false);
    expect(isPrimo(0)).toBe(false);
  });
  test("33.2 deve retornar false para número composto", () => {
  ...
  test("36.1 deve fixar no mínimo quando menor que o intervalo", () => {
    expect(clamp(-1, 0, 10)).toBe(0);
    expect(clamp(0, 0, 10)).toBe(0);
  });
  test("36.2 deve fixar no máximo quando maior que o intervalo", () => {
  ...
  test("37.1 deve retornar false quando não for divisível", () => {
    expect(isDivisivel(10, 3)).toBe(false);
  });
  test("37.2 deve retornar false quando divisor for 0 (comportamento do JS)", () => {
    expect(isDivisivel(10, 0)).toBe(false);
  });
  test("38. deve converter Celsius para Fahrenheit", () => {
    expect(celsiusParaFahrenheit(0)).toBe(32);
  });
  test("38.1 deve converter Celsius para Fahrenheit para valor diferente de 0", () => {
  ...
  test("39.1 deve converter Fahrenheit para Celsius para valor diferente de 32", () => {
    expect(fahrenheitParaCelsius(212)).toBe(100);
  });
  test("40. deve calcular o inverso de um número", () => {
    expect(inverso(4)).toBe(0.25);
  });
  test("40.1 deve lançar erro ao tentar inverter 0", () => {
  ...
  test("47.1 deve calcular a mediana de um array ímpar e DESordenado", () => {
    expect(medianaArray([10, 2, 1])).toBe(2);
  });
  test("47.2 deve calcular a mediana de um array par", () => {
    expect(medianaArray([1, 2, 3, 4])).toBe(2.5);
  });
  test("47.3 deve lançar erro ao calcular a mediana de um array vazio", () => {
    expect(() => medianaArray([])).toThrow("Array vazio не possui mediana.");
  });
  test("47.4 deve calcular a mediana de um array ímpar e DESordenado (verifica ordenação)", () => {
    expect(medianaArray([3, 1, 2])).toBe(2);
  });
  test("47.5 deve calcular a mediana de um array par e DESordenado (verifica ordenação)", () => {
    expect(medianaArray([5, 100, 1, 2])).toBe(3.5);
  });
